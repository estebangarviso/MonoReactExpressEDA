...
describe('E2E tests: Use cases for the API', () => {
  ...
  const newUsers = {
    salesman: {
      ...
      email: faker.internet
        .email(newUsersNames.salesman.firstName, newUsersNames.salesman.lastName)
        .toLocaleLowerCase(),
    ...
    customer: {
      ...
      email: faker.internet
        .email(newUsersNames.customer.firstName, newUsersNames.customer.lastName)
        .toLocaleLowerCase(),
      password: faker.internet.password(8),
      roleId: 2
    }
  ...
  const newArticles = [
    {
      sku: `${faker.commerce.product().toLowerCase()}-${faker.random.numeric(3)}`,
  ...
  describe('User registration and authentication', () => {
    it('should be able to create (sign up) a new customer', async () => {
      ...
      it(
        'should be able to login a customer and get an access tokens',
        async () => {
      ...
      it(
        'should be able to login a salesman and get an access tokens',
        async () => {
          const keys = ['accessToken', 'refreshToken']
          const {
            data: { message }
    ...
    describe('Salesman article creation', () => {
      it('salesman should be able to save a first new article', async () => {
        ...
        expect(response.data.message.shortDescription).toEqual(newArticles[0].shortDescription)
        ...
        it('salesman should be able to save a second new article', async () => {
          ...
          expect(response.data.message.shortDescription).toEqual(newArticles[1].shortDescription)
          expect(response.data.message.qtyStock).toEqual(newArticles[1].qtyStock)
        ...
        it('salesman should be able to save a third new article', async () => {
          ...
          expect(response.data.message.shortDescription).toEqual(newArticles[2].shortDescription)
          expect(response.data.message.qtyStock).toEqual(newArticles[2].qtyStock)
    ...
    describe('Users profiles', () => {
      it(
        'should be able to get customer profile data with access token',
        async () => {
        ...
        it(
          'should be able to get salesman profile data with access token',
          async () => {
            const response = await axios.get('/user/profile', {
              headers: {
                Authorization: `Bearer ${salesmanTokens.accessToken}`
    ...
    describe('Users orders and transactions', () => {
      test(
        'customer tries to bought an article without money, should be rejected',
        async () => {
          ...
          expect(
            String(response.data.message).includes(`You don't have enough money to make this purchase.`)
          ).toBeTruthy()
        }
      ),
        it('the customer should be able to transfer cash to himself', async () => {
          ...
          expect(Number(response.data.message.amount)).toBe(Number(newAmountToDeposit))
        ...
        test('verify balances with user transactions and variables', async () => {
          ...
          expect(Number(salesmanResponse.data.message)).toBe(Number(balances.salesman.final))

          expect(customerResponse.status).toBe(200)
          expect(Number(customerResponse.data.message)).toBe(Number(balances.customer.final))
